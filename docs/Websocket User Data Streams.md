...
- **在瀏覽器解 protobuf**：前端若直接連 `*.api.pb` channel，可安裝 `protobufjs` 並引入官方 proto schema；確認 `protobufjs` 版本支援 proto3（常見的 v7 系列即可），並與後端生成的 schema 保持一致；如由後端轉 JSON，確保 Python 端已安裝 `protobuf`（`requirements.txt` 中已固定版本 `protobuf==4.25.1`）後再將資料推送到前端。
