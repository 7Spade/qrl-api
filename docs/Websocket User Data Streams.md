...
- **在瀏覽器解 protobuf**：前端若直接連 `*.api.pb` channel，可安裝 `protobufjs` 並引入官方 proto schema；如由後端轉 JSON，確保 Python 端已安裝 `protobuf`（`requirements.txt` 中已固定版本 `protobuf==4.25.1`）後再將資料推送到前端。
