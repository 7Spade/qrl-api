...
- **在瀏覽器解 protobuf**：前端若直接連 `*.api.pb` channel，可安裝 `protobufjs` 並引入官方 proto schema（來源：https://github.com/mexcdevelop/websocket-proto）；確認 `protobufjs` 版本支援 proto3（常見的 v7 系列即可）且與後端生成的 schema 保持一致。
