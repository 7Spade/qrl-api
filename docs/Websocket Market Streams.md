...
- **在瀏覽器解 protobuf**：若前端直接連到 `*.api.pb` channel，可在瀏覽器端安裝 `protobufjs@7.x`（支援 proto3），並引入官方 proto schema（來源：https://github.com/mexcdevelop/websocket-proto）；可優先使用 `protobufjs/light` 並搭配 tree-shaking 以降低瀏覽器 bundle 大小。
