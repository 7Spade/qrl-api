    ...
    def _strategy_phase(self, market_data: Dict[str, Any]) -> TradeSignal:
        ...
        try:
            ...
            if current_price >= avg_cost * config.ACTIVE_SELL_THRESHOLD:
                ...
                logger.info(f"Signal: SELL (Active position - price {profit_pct:.1f}% above cost)")
                return TradeSignal.SELL

            # Check if we should sell swing position (larger profit taking)
            if current_price >= avg_cost * config.SWING_SELL_THRESHOLD:
                profit_pct = ((current_price/avg_cost-1)*100)
                logger.info(f"Signal: SELL (Swing position - price {profit_pct:.1f}% above cost)")
                return TradeSignal.SELL
            ...
            if current_price < long_ma * config.DCA_DIP_THRESHOLD:
                if not config.ALLOW_BUY_ABOVE_COST and current_price > avg_cost:
                    logger.info("Price dip detected but above cost - HOLD (cost protection)")
                    return TradeSignal.HOLD

                dip_pct = ((1-current_price/long_ma)*100)
                logger.info(f"Signal: BUY (Price dip - {dip_pct:.1f}% below MA)")
            ...
            if last_trade_time:
                ...
                if hours_since_last >= config.DCA_INTERVAL_HOURS:
                    if not config.ALLOW_BUY_ABOVE_COST and current_price > avg_cost:
                        logger.info("Periodic DCA due but price above cost - HOLD (cost protection)")
                        return TradeSignal.HOLD
                ...
                if cost_data:
                    ...
                    if avg_cost > 0:
                        ...
                        if profit_percent < config.MIN_SELL_PROFIT_PERCENT:
                            min_profit = config.MIN_SELL_PROFIT_PERCENT
                            logger.warning(f"Profit {profit_percent:.2f}% below minimum {min_profit}%")
                            return False
