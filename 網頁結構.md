...
> 現況：`src/app/interfaces/http` 已建空骨架，現行路由仍在 `api/` 下運作；待用例搬遷後再切換。
> 更新：`main.py` 已改為透過 `interfaces/http` shim 匯入現有 router，行為保持不變。
...
# 
templates/
├── dashboard.html                  # 只負責 layout + container
│
└── static/
    └── js/
        ├── api/                    # 一檔 = 一個 HTTP endpoint
        │   ├── market/
        │   │   ├── get_price.js
        │   │   ├── get_klines.js
        │   │   ├── get_trades.js
        │   │   ├── get_orderbook.js
        │   │   └── get_exchange_info.js
        │   │
        │   ├── account/
        │   │   ├── get_balance.js
        │   │   ├── list_orders.js
