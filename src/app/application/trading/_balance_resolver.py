...
class BalanceResolver:
    ...
    async def get_usdt_balance(self) -> float:
        ...
        try:
            ...
            logger.warning(
                "Primary balance fetch failed, "
                "attempting cache fallback"
            )

        if usdt_balance <= 0 and hasattr(
            self.redis, "get_cached_account_balance"
        ):
            cached_balance = (
                await self.redis.get_cached_account_balance()
            )
            if cached_balance:
                usdt_balance = float(
                    cached_balance.get("balances", {})
                    .get("USDT", {})
                    .get("free", 0)
                )
        return usdt_balance
