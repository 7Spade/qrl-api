...
class TradingService:
    ...
    def __init__(
        ...
        self.price_resolver = price_resolver or PriceResolver(
            self.mexc, self.price_repo
        )
        self.balance_resolver = balance_resolver or BalanceResolver(
            self.mexc, self.redis
        )
        self.position_updater = position_updater or PositionUpdater(
            self.cost_repo, self.position_repo
        )
    ...
    async def execute_trade_decision(self, symbol: str = "QRLUSDT") -> Dict:
        ...
        try:
            ...
            if signal == "BUY":
                ...
                total_qrl = (
                    float(position_data.get("total_qrl", 0)) if position_data else 0
                )
                core_qrl = (
                    float(position_data.get("core_qrl", 0)) if position_data else 0
                )
            ...
            if signal == "BUY":
                ...
                new_cost_data = await self.cost_repo.update_after_buy(
                    buy_quantity=quantity, buy_price=current_price
                )
                # Update position
                new_position = {
        ...
        try:
            ...
            if position.get("has_position"):
                ...
                pnl_data = await self.cost_repo.calculate_pnl(
                    current_price=current_price, current_quantity=total_qrl
                )
            else:
                pnl_data = {}
