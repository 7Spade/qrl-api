...
Extracted for clearer responsibilities.
"""
...
def summarize_cost_data(
    ...
    """Summarize cost data with P&L calculations"""
    try:
        ...
        pnl = calculate_unrealized_pnl(
            avg_cost, current_price, current_quantity
        )
        pnl.update(
            {
...
def update_after_buy_values(
    ...
    """Update cost data after buy trade"""
    old_total_invested = safe_float(
        cost_data.get("total_invested")
    )
    realized_pnl = safe_float(cost_data.get("realized_pnl"))
...
def update_after_sell_values(
    ...
    """Update cost data after sell trade"""
    realized_pnl = safe_float(cost_data.get("realized_pnl"))
    ...
    return {
        ...
        "total_invested": max(
            0,
            safe_float(cost_data.get("total_invested"))
            - cost_basis,
        ),
    }
